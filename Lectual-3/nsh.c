...
void setargs(char *cmd, char* argv[],int* argc)
{
    ...
    {
        // 跳过之前的空格
        while (strchr(whitespace,cmd[j])){	//cmd[j]出现在whitespace字符串中
            j++;
        }
        argv[i++]=cmd+j;        //argv[i]是指针，指向cmd[j]地址
        while (strchr(whitespace,cmd[j])==0){// 只要不是空格，就j++,找到下一个空格
            j++;
        }
        cmd[j]='\0';
    }
    argv[i]=0;//argv[]最后一个元素是0
    *argc=i;
}
...
void execPipe(char*argv[],int argc){
    ...
    if(fork()==0){
        ...
        close(fd[1]);
        // exec(argv[0],argv);
        runcmd(argv,i);//argc = i，只运行左半边命令argv[0——i]
    }else{
        ...
        runcmd(argv+i+1,argc-i-1);//argc = argc-（i+1），argv[i+1——argc]
    }
}
int main()
{
