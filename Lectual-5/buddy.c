...
bd_initfree_pair(int k, int bi, void *left, void *right) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  //if(bit_isset(bd_sizes[k].alloc, bi) !=  bit_isset(bd_sizes[k].alloc, buddy)) { //Lab buddy alloc注释掉
  if(bit_get(bd_sizes[k].alloc,bi)){
    ...
    /*if(bit_isset(bd_sizes[k].alloc, bi))
      ...
      lst_push(&bd_sizes[k].free, addr(k, bi));      // put bi on free list*///Lab buddy 注释掉
  }
  return free;
}
